...
class Net(nn.Module):
    ...
    def forward(self, state_input):
        ...
        x_act = F.log_softmax(self.act_fc1(x_act)).view(-1, self.board_height, self.board_width)
        # state value layers
...
class PolicyValueNet():
    ...
    def policy_value_fn(self, board):
        ...
        if self.use_gpu:
            ...
            act_probs = np.exp(log_act_probs.data.cpu().numpy().squeeze())
        else:
            log_act_probs, value = self.policy_value_net(torch.from_numpy(current_state).float())
            act_probs = np.exp(log_act_probs.data.numpy().squeeze())

        positions = tuple(zip(*legal_positions))
        act_probs = zip(legal_positions, act_probs[positions])
    ...
    def train_step(self, state_batch, mcts_probs, z_batch, lr):
        ...
        if self.use_gpu:
            ...
            z_batch = torch.FloatTensor(z_batch).cuda()
        else:
            state_batch = torch.FloatTensor(state_batch)
        ...
        policy_loss = -torch.mean(torch.sum(mcts_probs*log_act_probs, (1,2)))
        ...
        entropy = -torch.mean(
                torch.sum(torch.exp(log_act_probs) * log_act_probs,(1,2))
                )
